...
\only<1>{\[I = C\frac{{\mathrm d} V}{{\mathrm d} t}  + I_{\text{ione kanaler}}\]}
\only<2->{\[I = C\frac{{\mathrm d} V}{{\mathrm d} t}  + {\bf\color{logored}I_{\text{ione kanaler}}}\]}
    \vspace{-5mm}
    \begin{figure}
...
\begin{frame}{Å lage en beregningsmodel består av tre steg, å sette opp modelen, estimere parametre og beregne usikkerhetene}
    \begin{tikzpicture}[remember picture, overlay, font=\sffamily]
      \onslide<1-3>{\node [align=left, xshift=-0.35\textwidth,yshift=0.15\textwidth] (image3) at (current page.center)
            {$I = C\frac{{\mathrm d} V}{{\mathrm d} t}  + I_{\text{ione kanaler}}$};
